...
class PrintDialog(QDialog):
    ...
    def init_ui(self):
        ...
        # --- MODIFICATION: Scaling UI is completely removed ---
    ...
    # --- MODIFICATION: The print logic is now simplified and correct ---
    def process_and_print(self, printer):
        ...
        try:
            for i, page_num in enumerate(pages_to_print):
                ...
                # Step 1: Always find the content box to avoid printing large empty margins
                content_box = page.bound()
                clip_rect = content_box if not content_box.is_empty else page.rect

                # Step 2: Render the content area into a QImage at the selected print DPI
                ...
                # Step 3: Always scale the content to fit the paper (maintaining aspect ratio)
                page_rect_pixels = printer.pageRect(QPrinter.Unit.DevicePixel)
                target_size = q_image.size().scaled(page_rect_pixels.size().toSize(),
                                                    Qt.AspectRatioMode.KeepAspectRatio)

                # Step 4: Center the scaled image on the paper and draw
                x = (page_rect_pixels.width() - target_size.width()) / 2
                y = (page_rect_pixels.height() - target_size.height()) / 2

                target_rect = QRect(int(x), int(y), target_size.width(), target_size.height())
    ...
    # --- MODIFICATION: Preview logic is also simplified ---
    def render_current_preview_page(self, *args):
        ...
        try:
            ...
            # Always get the content box for a "Fit Page" preview
            content_box = page.bound()
            clip_rect = content_box if not content_box.is_empty else page.rect
            ...
            # Apply orientation for the preview
            if self.orientation_combo.currentText() == "Landscape":
                transform = QTransform().rotate(90)
                pixmap = pixmap.transformed(transform, Qt.TransformationMode.SmoothTransformation)

            # Final scaling to fit the preview widget
