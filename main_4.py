...
class PrintDialog(QDialog):
    def __init__(self, file_path, parent=None):
        ...
        # --- MODIFICATION: New data structure for paper sizes ---
        self.paper_sizes = [
            {'display': 'Letter (8.5 x 11.0 in)', 'id': QPageSize.PageSizeId.Letter},
            {'display': 'Legal (8.5 x 14.0 in)', 'id': QPageSize.PageSizeId.Legal},
            {'display': 'Legal (8.5 x 13.0 in)', 'id': None, 'size_in': QSizeF(8.5, 13.0)},  # Non-standard
    ...
    def init_ui(self):
        ...
        # --- MODIFICATION: Paper Size Section with Custom Inputs ---
        ...
        # Create a container for custom size widgets
        self.custom_size_widget = QWidget()
        custom_size_layout = QHBoxLayout(self.custom_size_widget)
        custom_size_layout.setContentsMargins(0, 5, 0, 0)  # Tweak spacing
        ...
        self.custom_size_widget.hide()  # Initially hidden
        # --- End of Modification ---
    ...
    # --- NEW FUNCTION to show/hide custom size inputs ---
    ...
    def execute_print(self):
        ...
        try:
            ...
            # --- MODIFICATION: Advanced Paper Size Logic ---
            ...
            if selected_paper_text == "Custom...":
                ...
                # Find the selected paper in our data structure
                paper_info = next((p for p in self.paper_sizes if p['display'] == selected_paper_text), None)
                if paper_info:
                    if paper_info['id'] is not None:
                        # Standard size with a known ID
                        printer.setPageSize(QPageSize(paper_info['id']))
                    elif 'size_in' in paper_info:
                        # Non-standard size defined by dimensions (e.g., Legal 8.5x13)
